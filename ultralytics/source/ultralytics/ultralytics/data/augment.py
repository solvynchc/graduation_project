...
class LetterBox:
    ...
    def __call__(self, labels: dict[str, Any] | None = None, image: np.ndarray = None) -> dict[str, Any] | np.ndarray:
        ...
        if aux_mask is not None:
            aux_mask = cv2.copyMakeBorder(aux_mask, top, bottom, left, right, cv2.BORDER_CONSTANT, value=0)
            labels["aux_mask"] = aux_mask
...
class Format:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if "aux_mask" in labels:
            aux_mask = labels["aux_mask"]
            if aux_mask.ndim == 2:
                aux_mask = aux_mask[None]
            labels["aux_mask"] = torch.from_numpy((aux_mask > 0).astype(np.float32))
        if "aux_edge" in labels:
            aux_edge = labels["aux_edge"]
            if aux_edge.ndim == 2:
