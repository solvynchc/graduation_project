...
class RawMaskYOLODataset(YOLODataset):
    """YOLO segmentation dataset with optional raw binary mask loading for auxiliary supervision."""

    def __init__(self, *args, data: dict | None = None, **kwargs):
        self.raw_mask_dir = Path(data["raw_mask_dir"]) if data and data.get("raw_mask_dir") else None
    ...
    def _read_mask_from_dir(self, image_stem: str) -> np.ndarray | None:
        if not self.raw_mask_dir:
            return None
        mask_path = self.raw_mask_dir / f"{image_stem}{self.raw_mask_suffix}"
        if not mask_path.exists():
            return None
        return cv2.imread(str(mask_path), cv2.IMREAD_GRAYSCALE)

    def _read_mask_from_zip(self, image_stem: str) -> np.ndarray | None:
        if not self.raw_mask_zip or not self.raw_mask_zip.exists():
            return None

        archive_name = f"{image_stem}{self.raw_mask_suffix}"
        entry_name = f"{self.raw_mask_zip_root.rstrip('/')}/{archive_name}".lstrip("/")

        with zipfile.ZipFile(self.raw_mask_zip) as zf:
            try:
                raw_bytes = zf.read(entry_name)
            except KeyError:
                matches = [name for name in zf.namelist() if name.endswith(f"/{archive_name}") or name == archive_name]
                if not matches:
                    return None
                raw_bytes = zf.read(matches[0])

        encoded = np.frombuffer(raw_bytes, dtype=np.uint8)
        return cv2.imdecode(encoded, cv2.IMREAD_GRAYSCALE)

    def load_raw_mask(self, image_path: str, image_shape: tuple[int, int]) -> np.ndarray:
        image_stem = Path(image_path).stem
        mask = self._read_mask_from_dir(image_stem)
        if mask is None:
            mask = self._read_mask_from_zip(image_stem)
        if mask is None:
    ...
    @staticmethod
    def build_edge_mask(mask: np.ndarray, kernel_size: int = 3) -> np.ndarray:
        """Generate a thin binary edge target from a binary lesion mask."""
